...
int waiting[NPROC]; /** Process index if the process is waiting for some lock, or -1 if not waiting
                     */
...
void PrintLocks()
{
    ...
    for (int i = 0; i < MAXLOCKS; i++)
    {
        ...
        {
            printf("%d : P%d \n", (i + 1), locks[i].by_child);
        }
    }
}
/**
 *      ***TODO*** (4)
 * Print the wait list so that YOU see what your program is doing
 */
...
int CheckForDeadLock()
{
    for (int i = 0; i < NPROC; i++)
    {
        if ((waiting[i] != -1 ) && (locks[waiting[i]].by_child == (1-i))){
            if(locks[waiting[(1-i)]].by_child ==i){
...
int LockManager(int cid, struct msg_requestLock ChildRequest, int respond[NPROC])
{
    ...
    if (ChildRequest.Action == RELEASE)
    {
        ...
        for (int i = 0; i < MAXLOCKS; i++)
        {
            if (locks[i].marked == MARKED && locks[i].by_child == cid)
            {
                ...
                for (int j = 0; j < NPROC; j++)
                {
                    if (waiting[j] == i)
                    {
                        ...
                        waiting[j] = -1;
                        STAT.status = GRANTED;
                        write(respond[j], (char *)&STAT, sizeof(STAT));
                    }
                }
            }
        }
        ...
        { /* lock is not free */
            ...
            {   /* lock taken by another child */
                ...
                if (CheckForDeadLock() == DEADLOCK_DETECTED)
                {
                    ...
                    locks[t_lock].by_child = -1;
                    locks[t_lock].marked = NOTMARKED;
